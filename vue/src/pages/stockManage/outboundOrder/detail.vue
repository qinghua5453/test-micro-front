<template>
  <div class="invoice">
    <!-- 面包屑导航 -->
    <q-breadcrumb :data="['库存管理', '出库单', '出库详情']" />
    <!--表格-->
    <div class="tab-s">
      <ul class="item-ul">
        <li class="item-li">
      ...
      <el-table border v-show="inventoryData.goodsAttribute == 10" :data="list" ref="list" style="width: 100%" :fit="true" height="603" class="my-table">
        ...
        <el-table-column prop="num" label="出库数量" width="200" show-overflow-tooltip></el-table-column>
      </el-table>
      <el-table border v-show="inventoryData.goodsAttribute == 20" :data="list" ref="list" style="width: 100%" :fit="true" height="603" class="my-table">
        <el-table-column fixed label="序号" width="60" type="index" :index="pagerIndex" show-overflow-tooltip></el-table-column>
        <el-table-column prop="goodsName" label="存货名称" show-overflow-tooltip></el-table-column>
        <el-table-column prop="goodsCode" label="存货编码" show-overflow-tooltip></el-table-column>
        <el-table-column prop="unit" label="计量单位" show-overflow-tooltip>
...
export default {
  ...
  methods: {
    ...
    async getAssetList() {
      ...
      let url = outDetailApi + par.inventoryOrderId;
      let res = await outDetailFun(url, par);
