<template>
  <div class="invoice">
    ...
    <el-form :inline="true" :model="searchData" ref="searchData" class="demo-form-inline device-wrapper">
      ...
      <el-form-item label="状态" prop="status">
        <el-select size="small" v-model="searchData.status" placeholder="不限">
          <el-option label="不限" value></el-option>
          <el-option v-for="(key, value) in industryStatus" :key="key" :label="key" :value="value"></el-option>
        </el-select>
      </el-form-item>
    ...
    <!--表格-->
    <div class="tab-s">
      <el-button type="primary" class="el-icon-circle-plus btn-add" @click="goAddIndustry" v-if="getPerm('mer:industry:add')">
        发布行业经验
      </el-button>
      <el-table border :data="list" ref="list" style="width: 100%" :fit="true" height="603" class="my-table">
        ...
        <el-table-column prop="createName" label="创建人" width="100" show-overflow-tooltip></el-table-column>
        <el-table-column prop="status" label="状态" width="160" show-overflow-tooltip :formatter="formatterStatusType"></el-table-column>
        <el-table-column fixed="right" label="操作" width="120">
          <template slot-scope="scope">
            <el-tooltip content="发布" placement="top" effect="dark" v-if="scope.row.status == 0 && getPerm('mer:industry:release')">
              <svg class="icon" aria-hidden="true" @click="release(scope.row)">
                <use xlink:href="#icon-release"></use>
              </svg>
            </el-tooltip>
            <el-tooltip content="撤回" placement="top" effect="dark" v-if="scope.row.status == 1 && getPerm('mer:industry:back')">
              <svg class="icon" aria-hidden="true" @click="withdraw(scope.row)">
                <use xlink:href="#icon-withdraw"></use>
              </svg>
            </el-tooltip>
            <el-tooltip content="编辑" placement="top" effect="dark" v-if="scope.row.status == 0 && getPerm('mer:industry:edit')">
              <svg class="icon" aria-hidden="true" @click="editType(scope.row)">
                <use xlink:href="#icon-document-edit"></use>
              </svg>
            </el-tooltip>
            <el-tooltip content="删除" placement="top" effect="dark" v-if="getPerm('mer:industry:delete')">
...
import { industryStatus } from "@/utils/Mapping";
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    // ...
    next(vm => {
      if (from.path == "/editIndustry" || from.path == "/industryDetail") {
  ...
  methods: {
    ...
    formatterStatusType(row,column) {
      return industryStatus[row.status];
    },
    goAddIndustry(row) {
      this.$router.push({
        name: 'editIndustry',
    ...
    release(row) {
      this.$confirm('您确定发布该行业经验吗？', '提示').then(() => {
        releaseFun( {
            id: row.id
          })
          .then(resp => {
    ...
    withdraw(row) {
      this.$confirm('您确定撤回该行业经验吗？', '提示').then(() => {
        fallbackFun( {
            id: row.id
          })
          .then(resp => {
    ...
    handleDelete(row) {
      this.$confirm('您确定删除该行业经验吗？', '提示').then(() => {
        deleteFun( {
            id: row.id
          })
          .then(resp => {
    ...
    async getList() {
      ...
      if (this.getPerm("mer:industry:list")) {
        let res = await industryListFun(par);
