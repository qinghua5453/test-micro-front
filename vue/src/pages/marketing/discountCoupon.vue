<template>
    ...
    <div style="margin-top:40px;">
        <el-form :rules="rules" :model="discountCouponData"  ref="discountCouponData">
            <el-table border :data="discountCouponData.vouchers" style="width: 100%" class="my-table">
                <el-table-column prop="type" label="类型" fixed>
                    <template slot-scope="scope">
                        {{discountCouponData.vouchers[scope.$index].type === 5 ? '折扣券' : ''}}
                    </template>
                </el-table-column>
                ...
                <el-table-column label="生效日" width="160">
                    <template slot-scope="scope">
                        <el-form-item v-if="discountCouponData.vouchers[scope.$index].dateType === 0" :prop="'vouchers.' + scope.$index + '.startDays'" :rules='rules.startDays'>
                ...
                <el-table-column label="有效期" width="280">
                    <template slot-scope="scope">
                        <el-form-item v-if="discountCouponData.vouchers[scope.$index].dateType === 0" :prop="'vouchers.' + scope.$index + '.day'" :rules='rules.day'>
                             <el-input style="width:80%;" v-model="scope.row.day" :disabled="!isEditorAdd"/>&nbsp;&nbsp;天
                        </el-form-item>
                        <el-form-item v-if="discountCouponData.vouchers[scope.$index].dateType === 1" :prop="'vouchers.' + scope.$index + '.date'" :rules='rules.date'>
                            <el-date-picker
                ...
                <el-table-column label="减免上限" width="240">
                    <template slot-scope="scope">
                        <el-form-item :prop="'vouchers.' + scope.$index + '.maxDerate'" :rules='rules.maxDerate'>
                             <el-input style="width:80%;" v-model="scope.row.maxDerate" :disabled="!isEditorAdd"/>&nbsp;元
                        </el-form-item>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="functionDescribe" label="操作" width="150" v-if="isEditorAdd">
                    <template slot-scope="scope">
                        <el-button type="primary" size="mini" @click="handleActAddCoupon(scope.$index)" v-if="discountCouponData.vouchers[scope.$index].isShowCouponAddBtn"> + </el-button>
                        <el-button type="primary" size="mini" @click="handleRemoveActCoupon(scope.$index)" v-if="discountCouponData.vouchers.length > 1" >  - </el-button>
                    </template>
                </el-table-column>
            </el-table>
        </el-form>
...
export default {
  data() {
    ...
    var checkStartDays = (rule, value, callback) => {
      ...
      if (!reg_startDays.test(value)) {
        return callback(new Error("必须为大于或者等于0的整数"));
      } else {
        callback();
      }
    };
    ...
    var checkChance = (rule, value, callback) => {
      let reg = /^([1-9]\d*)(\s|$|\.\d{1,2}\b)/; // 验证概率大于或者等于0，并且包含两位小数
      if (!reg.test(value)) {
        return callback(new Error("必须大于或者等于0，允许两位小数"));
      } else {
        callback();
      }
    };
    ...
    var checkPrice = (rule, value, callback) => {
      let reg = /(^[1-9](\d+)?(\.\d{1,2})?$)|(^\d\.\d{1,2}$)/; // 验证减免上限大于0，并且包含两位小数
      if (!reg.test(value)) {
        return callback(new Error("必须大于0，最多允许两位小数"));
      } else {
        callback();
      }
    };
    return {
      ...
      rules: {
        ...
        maxDerate: [
          { required: true, message: "请填写减免上限", trigger: "blur" },
  ...
  methods: {
    ...
    handleActAddCoupon(index) {
      ...
      this.discountCouponData.vouchers.push(addActCoupon);
      this.isShowBtn(this.discountCouponData.vouchers);
    },
    // 移除活动优惠券
    handleRemoveActCoupon(index) {
      if (this.discountCouponData.vouchers.length > 1) {
        this.discountCouponData.vouchers.splice(index, 1);
      }
      this.isShowBtn(this.discountCouponData.vouchers);
      this.$refs.discountCouponData.clearValidate();
    },
    // 判断'+ - '按钮显示
    isShowBtn(list) {
    ...
    validateFullForm() {
      let flag = false;
      this.$refs["discountCouponData"].validate(valid => {
        flag = valid;
      });
      return flag;
    },
