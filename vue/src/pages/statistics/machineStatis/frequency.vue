...
<template>
  <section v-if="getPerm('statis:listFrequencyDevice')">
    <el-row>
      <el-col :span="24">
        <!--表单-->
        ...
        <div class="tab-s">
          <div class="chart-warp">
            <h2>台均次数</h2>
            <h3>（台均次数=当日有效订单数/当日有效设备数）</h3>
            <line-chart :legendData="lgendData" :xAxis="xAxis" :yAxis="yAxis" :series="series"></line-chart>
            <!--表格-->
            <h2 class="table-title">台均次数明细<el-button type="primary" class="export-xsl" @click="exportTable()">导出</el-button>
            </h2>
...
export default {
  ...
  created () {
    if (this.getPerm("statis:listFrequencyDevice")) {
      this.getShopType();
      this.getMachineType();
      this._getList();
      this.getchartList();
  ...
  methods: {
    ...
    getchartList () {
      ...
      this.series[0].data = [];
      this.series[1].data = [];
      this.$axios.post(Api.DEVICE_FREQUENCY_STATIS, query).then(res => {
    ...
    exportTable () {
      ...
      this.$refs.searchData.validate(valid => {
        if (valid) {
          ...
          query.excel = true;
          exportExcel(
            Api.DEVICE_FREQUENCY_STATIS,
            `台均次数明细${query.startDate}-${query.endDate}.xlsx`,
            query
          );
        }
      });
