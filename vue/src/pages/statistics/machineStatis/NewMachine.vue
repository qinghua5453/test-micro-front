...
<template>
  <section v-if="getPerm('statis:listAddDevice')">
    <el-row>
      <el-col :span="24">
        <!--表单-->
        ...
        <div class="tab-s">
          <div class="chart-warp">
            ...
            <!--表格-->
            <h2 class="table-title">新增设备数据明细<el-button type="primary" class="export-xsl" @click="exportTable()">导出</el-button>
            </h2>
            <el-table :data="list" border tooltip-effect="dark" center="true" class="my-table" height="409">
              ...
              <el-table-column prop="deviceAddCount" label="新增设备数" show-overflow-tooltip>
                <template slot-scope="scope">
                  <span>{{scope.row.deviceAddCount | numFormat}}</span>
                </template>
              </el-table-column>
            </el-table>
            <!--分页-->
...
export default {
  ...
  data () {
    return {
      ...
      shopTypeList: [], // 店铺类型
      firstTypes: [], // 一级类型
      lgendData: [{ name: "新增设备数", icon: "rect" }],
      xAxis: {
        data: [],
        axisTick: {
          alignWithLabel: true
  ...
  created () {
    if (this.getPerm("statis:listAddDevice")) {
      this.getShopType();
      this.getMachineType();
      this._getList();
      this.getchartList();
  ...
  methods: {
    ...
    getchartList () {
      ...
      this.xAxis.data = [];
      this.series[0].data = [];
      this.$axios.post(Api.DEVICE_NEW_STATIS, query).then(res => {
        res.items = res.items.sort(ortId);
        res.items.forEach(item => {
          this.xAxis.data.push(item.date);
          this.series[0].data.push(item.deviceAddCount);
        });
      });
    },
    //明细列表
    ...
    exportTable () {
      ...
      this.$refs.searchData.validate(valid => {
        if (valid) {
          ...
          query.excel = true;
          exportExcel(
            Api.DEVICE_NEW_STATIS,
            `新增设备数据明细${query.startDate}-${query.endDate}.xlsx`,
            query
          );
        }
      });
