...
<template>
  <section v-if="getPerm('statis:listVipUser')">
    <el-row>
      <el-col :span="24">
        <!--表单-->
        ...
        <div class="tab-s">
          <div class="chart-warp">
            <h2>VIP用户数据</h2>
            <line-chart :id="vip" style="width:100%;" :legendData="lgendData" :grid="grid" :xAxis="xAxis" :yAxis="yAxis" :series="series"></line-chart>
            <!--表格-->
            <h2 class="table-title">VIP用户数据明细<el-button type="primary" class="export-xsl" @click="exportTable()">导出</el-button>
            </h2>
            <el-table :data="list" border tooltip-effect="dark" center="true" class="my-table" height="409">
              ...
              <el-table-column prop="vipUserCount" label="VIP用户数" show-overflow-tooltip>
                <template slot-scope="scope">
                  <span>{{scope.row.vipUserCount | numFormat}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="vipAddUserCount" label="VIP新增数" show-overflow-tooltip>
                <template slot-scope="scope">
                  <span>{{scope.row.vipAddUserCount | numFormat}}</span>
                </template>
              </el-table-column>
            </el-table>
            <!--分页-->
...
export default {
  ...
  data () {
    return {
      ...
      vip: "vip",
      lgendData: [
        { name: "vip用户数", icon: "rect" },
        { name: "vip新增数", icon: "rect" }
      ],
      grid: {
        left: "5%", //默认是80px
        right: "5%", //默认是60px
  ...
  created () {
    if (this.getPerm("statis:listVipUser")) {
      this._getList();
      this.getchartList();
    }
  },
  methods: {
    ...
    getchartList () {
      ...
      this.series[0].data = [];
      this.series[1].data = [];
      this.$axios.post(Api.USER_VIP_STATIS, query).then(res => {
        res.items = res.items.sort(ortId);
        res.items.forEach(item => {
          this.xAxis.data.push(item.date);
          this.series[0].data.push(item.vipUserCount);
          this.series[1].data.push(item.vipAddUserCount);
        });
        let machineMax =
        ...
        let penMax =
          calMax(this.series[1].data) > 0 ? calMax(this.series[1].data) : 1; //订单Y轴最大值
        let penMin = calMin(this.series[1].data);

        this.$set(this.yAxis[1], "max", penMax);
        this.$set(this.yAxis[1], "min", penMin);
        this.$set(this.yAxis[1], "interval", (penMax - penMin) / 5);
      });
    },
    //明细列表
    _getList () {
    ...
    exportTable () {
      ...
      this.$refs.searchData.validate(valid => {
        if (valid) {
          ...
          exportExcel(
            Api.USER_VIP_STATIS,
            `VIP用户数据明细${query.startDate}-${query.endDate}.xlsx`,
            query
          );
        }
      });
