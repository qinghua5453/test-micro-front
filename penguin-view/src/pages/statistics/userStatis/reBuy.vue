...
<template>
  <section v-if="getPerm('statis:listRebuyUser')">
    <el-row>
      <el-col :span="24">
        <!--表单-->
        ...
        <div class="tab-s">
          <div class="chart-warp">
            <h2>复购用户数据</h2>
            <h3>（复购用户是指统计周期（如7天）内的下单用户中存在历史订单记录的用户）</h3>
            <line-chart :id="rebuy" style="width:100%;" :legendData="lgendData" :xAxis="xAxis" :yAxis="yAxis" :series="series"></line-chart>
            <!--表格-->
            <h2 class="table-title">复购用户数据明细<el-button type="primary" class="export-xsl" @click="exportTable()">导出</el-button>
            </h2>
            <el-table :data="list" border tooltip-effect="dark" center="true" class="my-table" height="409">
              ...
              <el-table-column prop="userRebuy7Count" label="近7日复购用户数" show-overflow-tooltip>
                <template slot-scope="scope">
                  <span>{{scope.row.userRebuy7Count | numFormat}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="userRebuy7BeforeCount" label="前7日下单用户" show-overflow-tooltip>
                <template slot-scope="scope">
                  <span>{{scope.row.userRebuy7BeforeCount | numFormat}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="userRebuy7Ratio" label="7日复购率" show-overflow-tooltip></el-table-column>
            </el-table>
            <!--分页-->
...
export default {
  ...
  data () {
    return {
      ...
      rebuy: "rebuy",
      lgendData: [
        { name: "近7日复购用户数", icon: "rect" },
        { name: "前7日下单用户", icon: "rect" }
      ],
      xAxis: {
        data: [],
        axisTick: {
  ...
  created () {
    if (this.getPerm("statis:listRebuyUser")) {
      this._getList();
      this.getchartList();
    }
  },
  methods: {
    ...
    getchartList () {
      ...
      this.series[0].data = [];
      this.series[1].data = [];
      this.$axios.post(Api.USER_REBUY_STATIS, query).then(res => {
        res.items = res.items.sort(ortId);
        res.items.forEach(item => {
          this.xAxis.data.push(item.date);
          this.series[0].data.push(item.userRebuy7Count);
          this.series[1].data.push(item.userRebuy7BeforeCount);
        });
      });
    },
    //明细列表
    ...
    exportTable () {
      ...
      this.$refs.searchData.validate(valid => {
        if (valid) {
          ...
          query.excel = true;
          exportExcel(
            Api.USER_REBUY_STATIS,
            `复购用户数据明细${query.startDate}-${query.endDate}.xlsx`,
            query
          );
        }
      });
