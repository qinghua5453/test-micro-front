<template>
  <section v-if="getPerm('dal:order:compensateList')">
    <el-row>
      <el-col :span="24">
        <!-- 面包屑导航 -->
        <q-breadcrumb :data="['订单管理', '补偿记录']" />
        <!--表单-->
        <el-form :inline="true" :model="searchData" class="demo-form-inline" ref="searchData">
          ...
          <el-form-item v-if="getPerm('dal:order:compensateExcel')">
        ...
        <div class="tab-s">
          ...
          <el-table :data="list" border tooltip-effect="dark" center="true" class="my-table" height="740">
            ...
            <el-table-column label="补偿金额" width="100" show-overflow-tooltip>
          ...
          <!--补偿详情-->
          <el-dialog class="dialog-shop" title="补偿详情" :visible.sync="isShowCompensateDetailDialog" @close="isShowCompensateDetailDialog = false">
            <el-form :model="compensateDetail" label-width="96px">
              <el-form-item label="订单编号：" class="form-item-m">
                <span>{{compensateDetail.orderNo}}</span>
              </el-form-item>
              <el-form-item label="补偿金额：" class="form-item-m">
                <span>{{compensateDetail.price}}</span>
              </el-form-item>
              <el-form-item label="故障描述：" class="form-item-m">
                <span>{{compensateDetail.fault}}</span>
              </el-form-item>
              <el-form-item label="故障图片：" class="form-item-m">
                <img v-if="compensateDetail.imageUrl" v-for="(item,index) in compensateDetail.imageUrl" :key="index" :src="item" width="146" height="146" @click="showImg(item)">
              </el-form-item>
            </el-form>
          </el-dialog>
          <!-- 查看大图 -->
  ...
  export default {
    ...
    data () {
      return {
        ...
        isShowCompensateDetailDialog: false,
        compensateDetail: {
    ...
    created () {
      if (this.getPerm("dal:order:compensateList")) {
        this.getOrderCompensateList();
    ...
    methods: {
      ...
      //订单列表
      getOrderCompensateList () {
        this.apiUrl = Api.COMPENSATE_LIST;
      ...
      getOrderDetails (id) {
        return this.$axios
          .post(Api.COMPENSATE_DETAILS, {
            id: id
          })
          .then(res => {
            this.compensateDetail = res;
          });
      },
      openOrderDetailDialog (id) {
        if (this.getPerm("dal:order:compensateDetails")) {
          this.getOrderDetails(id).then(
            () => (this.isShowCompensateDetailDialog = true)
          );
        }
      },
      // 查看原图
      ...
      exportTable () {
        this.$refs.searchData.validate(valid => {
          if (valid) {
            ...
            query.endTime = query.time ? query.time[1] : null;
            query.time = null;
            exportExcel(Api.COMPENSATE_EXPORT, "补偿记录总览.xlsx", query);
          }
        });
      }
    },
