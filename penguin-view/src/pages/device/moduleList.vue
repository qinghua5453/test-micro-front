<template>
  <div class="page">
    ...
    <el-form :inline="true" :model="searchData" ref="searchData" class="demo-form-inline device-wrapper">
      ...
      <el-form-item label="RRPC" prop="isRRPC">
        <el-select size="small" v-model="searchData.isRRPC" placeholder="全部">
          <el-option label="不限" value=""></el-option>
          <el-option label="不支持" value="0"></el-option>
          <el-option label="支持" value="1"></el-option>
        </el-select>
    ...
    <div class="tab-s">
      <el-table border :data="list" ref="list" style="width: 100%" :fit="true" height="603" class="my-table">
        ...
        <el-table-column fixed prop="imei" label="IMEI" width="140" show-overflow-tooltip></el-table-column>
        <el-table-column prop="firmwareVersion" label="IMEI版本号" width="330" show-overflow-tooltip></el-table-column>
        <el-table-column prop="ccid" label="CCID" width="180" show-overflow-tooltip></el-table-column>
        <el-table-column prop="company" label="公司" width="60">
          <template slot-scope="scope">
            <span>{{scope.row.company | MachineCompany}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="thirdStatus" label="阿里状态" width="60">
          <template slot-scope="scope">
            <span>{{scope.row.thirdStatus | MachineAliStatus}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="status" label="模块状态" width="60">
          <template slot-scope="scope">
            <span>{{scope.row.status | modelImeiStatus}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="computeStatus" label="计算状态" width="60">
          <template slot-scope="scope">
            <span>{{scope.row.computeStatus | modelComputeStatus}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="bindStatus" label="是否绑定" width="60">
          <template slot-scope="scope">
            <span>{{scope.row.bindStatus | modelBindStatus}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="signals" label="信号值" width="50">
          <template slot-scope="scope">
            <span :class="{signalred:scope.row.signals < 18}">{{scope.row.signals}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="network" label="网络" width="50">
          <template slot-scope="scope">
            <span>{{scope.row.network | netWorkStatus}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="supportBle" label="蓝牙" width="60">
          <template slot-scope="scope">
            <span>{{scope.row.supportBle | modelSupportStatus}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="supportRRPC" label="RRPC" width="60">
          <template slot-scope="scope">
            <span>{{scope.row.supportRRPC | modelSupportStatus}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="regTime" label="注册时间" min-width="160"></el-table-column>
        <el-table-column fixed="right" label="操作" width="80">
          <template slot-scope="scope">
            ...
            <el-tooltip content="下发指令" placement="top" effect="dark">
              <i class="icon el-icon-s-promotion" style="color:#00A0E8" @click="openCmdDialog(scope.row.imei)"></i>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!--下发指令-->
    <cmd :visible.sync="isShowCmdDialog" v-if="isShowCmdDialog" :imei="iotLogId"></cmd>
  </div>
</template>
