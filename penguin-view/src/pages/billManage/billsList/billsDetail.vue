<template>
  <div class="invoice">
    ...
    <el-form :inline="true" :model="searchData" ref="searchData" class="demo-form-inline device-wrapper">
      ...
      <el-form-item>
        <a class="el-button" @click="exportTable()" v-if="getPerm('dal:billsDetail:excel')">导出</a>
      </el-form-item>
    </el-form>
    <!--表格-->
    <div class="tab-s">
      <el-table border :data="list" ref="list" :summary-method="getSummaries" show-summary style="width: 100%" :fit="true" height="603" class="my-table">
        ...
        <el-table-column prop="settlementState" label="状态" show-overflow-tooltip :formatter="formatterStatus">
        </el-table-column>
      </el-table>
      <!--分页-->
      <div class="block">
...
export default {
  ...
  created() {
    if (this.$route.query.billNo) {
      ...
      this.searchData.billStartDate = moment().subtract(6, 'months').format('YYYY-MM-DD 00:00:00'),
      this.searchData.billEndDate = moment().format('YYYY-MM-DD 23:59:59');
      this.searchData.time = [this.searchData.billStartDate,this.searchData.billEndDate];
    }
  ...
  methods: {
    ...
    formatterStatus(row, column) {
      return billStatus[row.settlementState];
    },
    remoteMethod(query) {
      if (query) {
        this.getShop(query);
    ...
    async getList() {
      ...
      if (this.getPerm("dal:billsDetail:list")) {
